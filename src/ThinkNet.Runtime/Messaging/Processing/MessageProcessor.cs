...
namespace ThinkNet.Messaging
{
    public class MessageProcessor : DisposableObject, IProcessor
    {
        ...
        private void OnProcessed(object message)
        {
            ...
            if (@event!= null && @event.Events.IsEmpty()) {
                commandResultManager.NotifyCommandCompleted(@event.CommandId, CommandStatus.NothingChanged, null);
                return;
            }
        }

        private void OnException(object message, Exception ex)
        {
            var command = message as ICommand;
            if (command != null) {
